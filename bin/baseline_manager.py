...
class BaselineManager:
    ...
    def save_baseline(
        self, category: str, data: Dict[str, Any], reason: str = "approved"
    ) -> bool:
        """
    ...
    def compare_with_baseline(
        self, category: str, new_data: Dict[str, Any]
        ...
        if category == "drift":
            has_changes = bool(
                diff.get("added_files")
                or diff.get("removed_files")
                or diff.get("modified_files")
            )
        elif category == "app":
            has_changes = bool(
                diff.get("added_apps")
                or diff.get("removed_apps")
                or diff.get("updated_apps")
            )
    ...
    def _generate_diff(
        self, category: str, old_data: Dict, new_data: Dict
    ) -> Dict[str, Any]:
                ...
                if n in new_apps:
                    ...
                    if isinstance(old_item, dict) and isinstance(new_item, dict):
                        ...
                        if old_item.get("size") != new_item.get("size"):
                            changes.append(
                                f"size: {old_item.get('size')} -> {new_item.get('size')}"
                            )
                        if old_item.get("permissions") != new_item.get("permissions"):
                            changes.append(
                                f"permissions: {old_item.get('permissions')} -> {new_item.get('permissions')}"
                            )
                        if old_item.get("target") != new_item.get("target"):
                            changes.append(
                                f"target: {old_item.get('target')} -> {new_item.get('target')}"
                            )
                        if old_item.get("subtype") != new_item.get("subtype"):
                            changes.append(
                                f"subtype: {old_item.get('subtype')} -> {new_item.get('subtype')}"
                            )
                        if old_item.get("type") != new_item.get("type"):
                            changes.append(
                                f"type: {old_item.get('type')} -> {new_item.get('type')}"
                            )
                        if changes:
    ...
    def save_golden_snapshot(
        self, report_data: Dict[str, Any], report_label: str = ""
    ) -> bool:
        ...
        # Atomic write
        tmp_fd, tmp_path = tempfile.mkstemp(prefix=".golden_", dir=self.baseline_dir)
        try:
            with os.fdopen(tmp_fd, "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    new_snapshot = {
        ...
        "app": {
            "applications": [{"name": "bash", "version": "5.1.16"}]
        },  # version changed
