...
class ChangeApprover:
    ...
    def record_pending_changes(
        self, category: str, changes: Dict[str, Any], timestamp: str
    ) -> None:
        """
    ...
    def approve_changes(
        self, category: Optional[str] = None, timestamp: Optional[str] = None
    ) -> bool:
        """
    ...
    def generate_approval_html(self) -> str:
        ...
        html += (
            "<tr><th>Category</th><th>Changes</th><th>Status</th><th>Action</th></tr>"
        )
...
def generate_change_diff(old_data: Dict, new_data: Dict) -> Dict[str, List[Dict]]:
    ...
    for key in new_keys - old_keys:
        diff["added"].append({"key": key, "new_value": new_data[key]})

    # Removed items
    for key in old_keys - new_keys:
        diff["removed"].append({"key": key, "old_value": old_data[key]})

    # Modified items
    for key in old_keys & new_keys:
        if old_data[key] != new_data[key]:
            diff["modified"].append(
                {"key": key, "old_value": old_data[key], "new_value": new_data[key]}
            )
