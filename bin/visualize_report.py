...
def load_minimal_config(config_path: str) -> Dict[str, Any]:
    ...
    if missing:
        raise VisualizeError(
            f"Config is missing required key(s) for this tool: {', '.join(missing)}"
        )
    return raw
            ...
            for full_path in entries:
                ...
                try:
                    if os.path.islink(full_path):
                        ...
                        item["permissions"] = oct(os.stat(full_path).st_mode)[-3:]
                        item["mtime"] = datetime.fromtimestamp(
                            os.path.getmtime(full_path), tz=timezone.utc
                        ).isoformat()

                        if full_path.endswith((".py", ".sh", ".bash")):
                            item["subtype"] = "script"
...
def load_yaml_snapshot(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise VisualizeError(
            f"Failed to read/parse YAML snapshot {path}: {exc}"
        ) from exc
...
def _compute_threshold_data(
    ...
    for entry in entries:
        ...
        net_changes = (
            len(rep_net.get("modified_settings", {}))
            if (rep_net and isinstance(rep_net, dict))
            else 0
        )
        app_changes = 0
        ...
        if is_baseline:
            ...
            if approvals_dir:
                try:
                    ...
                    if curr_snap and not any(
                        r.get("snapshot_id") == os.path.basename(curr_snap)
                        for r in audit_store.history()
                    ):
            ...
            if store and basename:
                try:
                    ...
                    if rec:
                        ...
                        exceeded = rec.get("threshold_exceeded", False)
                except Exception:  # noqa: BLE001
                    pass
        else:
            if store and basename:
    ...
    # A report left undecided before the next run is stale — its diff no longer
    # describes the live host, so only the newest report may stay pending.
    newest = ""
    if entries and entries[-1].get("file"):
        newest = os.path.basename(entries[-1]["file"])
    if store and newest:
        try:
            for stale in store.auto_reject_superseded(newest):
                if stale in reports_meta:
                    reports_meta[stale]["status"] = "rejected"
                    reports_meta[stale]["auto_rejected"] = True
        except Exception:  # noqa: BLE001
            pass
...
def _fmt_entity_name(name: str, iso_ts: Optional[str] = None) -> str:
    ...
    stem = (
        ...
        else (name[:-4] if name.endswith(".txt") or name.endswith(".html") else name)
    )
    ext = name[len(stem) :]
    ...
    m_iso = re.match(
        r"^(.*?)_?(\d{4}[-_]\d{2}[-_]\d{2}T\d{2}[_:]\d{2}[_:]\d{2}(?:\.\d+)?(?:Z|[+-]\d{2}[_:]?\d{2})?)$",
        remainder,
        re.I,
    )
    if m_iso:
    ...
    if m_std:
        ...
        if sec is not None:
            try:
                ...
                return (
                    f"{prefix}{sid2}_{ts_str}{cnt_str}{ext}"
                    if sid2
                    else f"{prefix}{ts_str}{cnt_str}{ext}"
                )
            except ValueError:
                pass
        else:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--config", help=f"Path to config.json (default lookup: {DEFAULT_CONFIG_PATH})"
    )
    parser.add_argument("--report", help="Visualize one specific report JSON file")
    parser.add_argument(
        "--report-dir", help="Reports directory (overrides config.json)"
    )
    ...
    parser.add_argument(
        "--output",
        help="Output HTML file path (default: report_dir/dashboard/dashboard_<server_id>_<ts>.html)",
    )
    parser.add_argument("--title", help="Custom dashboard title")
    parser.add_argument(
        "--app-dir",
        help="Application directory to scan for the Application Snapshots tab (optional, e.g. /bin)",
    )
    parser.add_argument(
        "--network-snapshot",
        help="Path to a YAML network snapshot file (e.g. Output/config-snapshot.yaml written by collect_network_snapshot.sh) for the Network Settings tab (optional)",
    )
    parser.add_argument(
        "--network-diff",
        help="Path to a JSON file containing the network baseline diff (written by run_all.py). When present, changed network properties are highlighted in the dashboard.",
    )
    parser.add_argument(
        "--app-diff",
        help="Path to a JSON file containing the application baseline diff (written by run_all.py). When present, changed application binaries/scripts are highlighted in the dashboard.",
    )
    parser.add_argument(
        "--version", action="version", version=f"visualize_report {SCRIPT_VERSION}"
    )
    ...
    try:
        ...
        if args.report:
            ...
            _approvals_dir = os.path.join(
                os.path.dirname(os.path.abspath(report_dir)),
        ...
        for path in report_paths:
            ...
            entries.append(
                {"file": path, "label": _report_label(path, report), "data": report}
            )
        ...
        if args.network_snapshot:
            if not os.path.isfile(args.network_snapshot):
                raise VisualizeError(
                    f"Network snapshot file not found: {args.network_snapshot}"
                )
        ...
        stable_path = os.path.join(
            out_dir, f"latest_dashboard_{server_id_for_title}.html"
        )
