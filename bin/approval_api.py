...
class ApprovalAPIHandler:
    ...
    def approve_changes(
        ...
        try:
            ...
            manager.save_baseline(
                category, snapshot_data, reason=reason or "user_approved"
            )

            # Record approval response
            self._record_response(
                category, "approved", snapshot_data, user=user, reason=reason
            )
    ...
    def reject_changes(
        self, category: str, user: str = "dashboard_user", reason: str = "user_rejected"
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            return (
                True,
                f"❌ {category} changes rejected. Previous baseline remains active.",
            )
    ...
    def get_approval_status(self) -> Dict[str, Any]:
        ...
        for category in ["drift", "app", "network"]:
            if category in pending and pending[category].get("status") == "pending":
                ...
                if category == "app" and isinstance(changes_data, dict):
                    change_count = (
                        len(changes_data.get("added_apps", []))
                        + len(changes_data.get("removed_apps", []))
                        + len(changes_data.get("updated_apps", []))
                    )
                elif category == "network" and isinstance(changes_data, dict):
                    change_count = len(changes_data.get("modified_settings", {}))
                elif category == "drift" and isinstance(changes_data, dict):
                    if (
                        "added_files" in changes_data
                        or "removed_files" in changes_data
                        or "modified_files" in changes_data
                    ):
                        change_count = (
                            len(changes_data.get("added_files", []))
                            + len(changes_data.get("removed_files", []))
                            + len(changes_data.get("modified_files", []))
                        )
                    elif (
                        "added" in changes_data
                        or "deleted" in changes_data
                        or "modified" in changes_data
                    ):
                        change_count = (
                            len(changes_data.get("added", []))
                            + len(changes_data.get("deleted", []))
                            + len(changes_data.get("modified", []))
                        )
                    else:
            ...
            elif category in pending and pending[category].get("status") in (
                "approved",
                "rejected",
            ):
                category_data = pending[category]
                status["categories"][category] = {
                    ...
                    "changes": 0,
                    "timestamp": category_data.get("response_at")
                    or category_data.get("timestamp"),
                }
            else:
                # No pending changes for this category
                status["categories"][category] = {"status": "no_changes", "changes": 0}
...
def handle_approval_request(
    request_data: Dict[str, Any], handler: ApprovalAPIHandler
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    handler = ApprovalAPIHandler(
        "web01", "/else/serversnap/approvals", "/else/serversnap/baselines"
    )
    ...
    reject_request = {"action": "reject", "category": "network"}
